...
<script>
  ...
  function startPolling(jobID) {
    fileNameDisplay.textContent = 'Checking status for uploaded image...';

    return new Promise((resolve, reject) => {
      const intervalId = setInterval(() => {
        // Send GET request to the status/retrieval endpoint
        fetch(`http://localhost/api/get-processed-img/${jobID}`)
          .then(response => {
            if (!response.ok) {
              // job ID not found or other non-200 error
            ...
            .then(data => {
              ...
              if (status === 'complete') {
                clearInterval(intervalId);
                // resolve the promise with the final URL string
                const imageURL = data.image_url; 
                console.log('Processing complete. Image URL:', imageURL);
                resolve(imageURL);     
              }
              else if (status === 'failed') {
                clearInterval(intervalId);
                reject(new Error(`Processing failed for Job ID ${jobID}.`));
              }
              else {
                // Status is 'queued' or 'processing'. Interval continues.
                fileNameDisplay.textContent = `Status: ${status}`;
              }
            })
            .catch(error => {
              clearInterval(intervalId);
  ...
  function processFile(file) {
    ...
    return fetch('http://localhost/api/upload-img', {
        method: 'POST',
        body: formData
    })
    .then(response => {
      ...
      if (!response.ok) {
        ...
        // expect JSON response containing job ID
        return response.json(); 
    })
    ...
    .then(data => {
        ...
        // update status
        fileNameDisplay.textContent = 'Image received. Queued for processing. Please wait.'

        // begin polling, waiting for the processed image
        return startPolling(jobID)
    })
    .then(imageURL => {
      fileNameDisplay.textContent = 'Processing complete!';

      const imgPreview = document.createElement('img');
      imgPreview.src = imageURL; // use the final URL string
      imgPreview.className = 'img-preview';
